...
def test_net(tester, dets, det_range, gpu_id):
    ...
    while img_start < det_range[1]:
        ...
        for batch_id in range(0, len(cropped_data), cfg.test_batch_size):
            ...
            #len(imgs):1
            # forward
            heatmap = tester.predict_one([imgs])[0] #(1, 64, 48, 4)
        ...
        kps_result[:, :, 2] = 1
        kps_result = kps_result.reshape(-1,cfg.num_kps*3) #[1, 12]
...
def test(test_model):
    ...
    def func(gpu_id):
        ...
        range = [ranges[gpu_id], ranges[gpu_id + 1]] #[0,999]
