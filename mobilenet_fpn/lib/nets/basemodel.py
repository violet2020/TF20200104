...
def mobilenet_v2_fpn(inp, is_train, conv_width=1.0):
    ## mobilenet v2
    mindepth = 32 #???
    #pdb.set_trace()   
...
'''
def mobilenet_v2_fpn_111(inp, conv_width=1.4):
    ## mobilenet v2
    mindepth = 32 #???
    #pdb.set_trace()   
    with tf.variable_scope('MobilenetV2') as scope:
        with slim.arg_scope(
                mobv.training_scope(is_training=True, bn_decay=0.9997)), \
                slim.arg_scope([mobilenet.depth_multiplier], min_depth=mindepth):
            with (slim.arg_scope(conv_hyperparams_fn())):

                _, image_features = mobv.mobilenet_base(
                    inp,
                    final_endpoint='layer_19',
                    depth_multiplier=conv_width,
                    scope=scope)
        ## fpn
        fpn_max_level = 7 #7
        fpn_min_level=2 #3
        ...
        with slim.arg_scope(conv_hyperparams_fn()):
            with tf.variable_scope('fpn'):
                feature_blocks = ['layer_4', 'layer_7', 'layer_14', 'layer_19']
                base_fpn_max_level = min(fpn_max_level, 5)
                feature_block_list = []
                for level in range(fpn_min_level, base_fpn_max_level + 1):# 3,4,5
                    feature_block_list.append(feature_blocks[level - 2]) #['layer_7', 'layer_14', 'layer_19']
                ...
                feature_maps = []
                for level in range(fpn_min_level, base_fpn_max_level + 1): # 6 7
                    feature_maps.append(fpn_features['top_down_{}'.format(
                        feature_blocks[level - 2])]) # 2
